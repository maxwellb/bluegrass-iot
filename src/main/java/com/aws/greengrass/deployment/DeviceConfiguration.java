...
public class DeviceConfiguration {
    ...
    private void initializeNucleusComponentConfig() {
        ...
        kernel.getConfig().lookup(SERVICES_NAMESPACE_TOPIC, DEFAULT_NUCLEUS_COMPONENT_NAME,
                VERSION_CONFIG_KEY).dflt(FALLBACK_VERSION);
    ...
    public void initializedNucleusFromRecipe(KernelAlternatives kernelAlts) {
        ...
        try {
            ...
            Path unpackArtifactPath = kernel.getContext().get(NucleusPaths.class).unarchiveArtifactPath(
                    new ComponentIdentifier(nucleusComponentName, componentVersion),
                    DEFAULT_NUCLEUS_COMPONENT_NAME.toLowerCase());
            copyUnpackedNucleusArtifacts(unpackDir, unpackArtifactPath);
            ...
            logger.atError().log("Unable to load Nucleus lifecycle from build recipe file", e);
        }
    }

    private void copyUnpackedNucleusArtifacts(Path src, Path dst) throws IOException {
        ...
        Files.walkFileTree(src, new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                ...
                if (file.getFileName() != null && files.contains(file.getFileName().toString())) {
                    Files.copy(file, dst.resolve(relativeFile), NOFOLLOW_LINKS, REPLACE_EXISTING, COPY_ATTRIBUTES);
                }
                return FileVisitResult.CONTINUE;
            }
        });
